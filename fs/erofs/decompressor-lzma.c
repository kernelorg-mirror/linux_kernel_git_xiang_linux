		            ...
		            struct list_head *pagepool)
{
	...
	xz_dec_microlzma_reset(strm->state, outlen, inlen,
			       !rq->partial_decoding);
	...
	for (ni = 0, no = -1;;) {
		...
		if (strm->buf.out_pos == strm->buf.out_size) {
			...
			if (++no >= nrpages_out) {
				erofs_err(rq->sb, "decompressed data out of bound");
				err = -EFSCORRUPTED;
				break;
			}
			if (rq->out[no])
			...
			DBG_BUGON(!inlen);
			if (++ni >= nrpages_in) {
				erofs_err(rq->sb, "compressed data out of bound");
				err = -EFSCORRUPTED;
				break;
			}
		...
		if (!bounced && rq->out[no] == rq->in[ni]) {
			memcpy(strm->bounce, kin, PAGE_SIZE);
	...
	if (ni < nrpages_in)
		kunmap(rq->in[ni]);
	/* 4. push back to the global list */
	spin_lock(&z_erofs_lzma_lock);
	strm->next = z_erofs_lzma_head;
	z_erofs_lzma_head = strm;
	spin_unlock(&z_erofs_lzma_lock);
