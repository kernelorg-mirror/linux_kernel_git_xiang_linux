...
static struct page *z_erofs_bvset_flip(struct z_erofs_bvec_iter *iter, bool atomic)
{
	...
	DBG_BUGON(!nextpage);
	oldpage = z_erofs_bvec_iter_end(iter, atomic);
	iter->bvpage = nextpage;
	iter->bvset = atomic ? kmap_atomic(nextpage) : kmap(nextpage);
	iter->nr = (PAGE_SIZE - base) / sizeof(struct z_erofs_bvset);
	iter->cur = 0;
	return oldpage;
}
				...
				struct page **candidate_bvpage)
{
	...
	iter->bvset->bvec[iter->cur].page = page;
	iter->bvset->bvec[iter->cur++].offset = offset;
	return 0;
}
...
{
	if (iter->cur == iter->nr)
		*old_bvpage = z_erofs_bvset_flip(iter, true);
	else
		*old_bvpage = NULL;
				   ...
				   struct page *page, int offset)
{
	...
	while (fe->icur > 0) {
		if (!cmpxchg(&pcl->compressed_bvecs[--fe->icur].page,
			     NULL, page)) {
			pcl->compressed_bvecs[fe->icur].offset = offset;
			return true;
		}
	}
	return false;
			       ...
			       struct page *page, int offset, bool exclusive)
{
	...
	if (exclusive) {
		...
		if (fe->mode >= Z_EROFS_PCLUSTER_FOLLOWED &&
		    !fe->candidate_bvpage)
			fe->candidate_bvpage = page;
	}
	ret = z_erofs_bvec_enqueue(&fe->biter, page, offset,
				   &fe->candidate_bvpage);
	fe->pcl->vcnt += (ret >= 0);
	return ret;
}
...
static bool z_erofs_collector_end(struct z_erofs_decompress_frontend *fe)
{
	...
	z_erofs_bvec_iter_end(&fe->biter, false);
	mutex_unlock(&pcl->lock);
...
static int z_erofs_parse_decompressed_bvecs(struct z_erofs_pcluster *pcl,
				struct page **pages, struct page **pagepool)
{
	...
	old_bvpage = z_erofs_bvec_iter_end(&biter, true);
	if (old_bvpage)
		z_erofs_put_shortlivedpage(pagepool, old_bvpage);
	return err;
}

static struct page **z_erofs_parse_compressed_bvecs(struct erofs_sb_info *sbi,
			...
			struct page **pagepool, bool *overlapped)
{
	...
	/* XXX: vmap() forces a page array instead of an iterator */
				       ...
				       struct page **pagepool, int err)
{
	...
	err = z_erofs_parse_decompressed_bvecs(pcl, pages, pagepool);
	compressed_pages = z_erofs_parse_compressed_bvecs(sbi, pcl, pages,
						pagepool, &overlapped);
